...
public class SudokuDataContainer
{
    ...
    public SudokuDataContainer(int level, int[,] solutionBoard, int[,] playableBoard, Dictionary<Point, int> deletedPositions)
    {
        this.level = level;
        this.solutionBoard = solutionBoard.Cast<int>().ToArray();
        this.playableBoard = playableBoard.Cast<int>().ToArray();
    ...
    public Dictionary<Point, int> GetDeletedPositionsDictionary() => GetDeletedPositions();
    #endregion
    ...
    public Dictionary<string, int> ConvertDeletedPositions(Dictionary<Point, int> deletedPositions)
    {
    ...
    /// </summary>
    public Dictionary<Point, int> GetDeletedPositions()
    {
        var converted = new Dictionary<Point, int>();
        foreach (var kvp in deletedPositions)
        {
            ...
            converted[new Point { X = x, Y = y }] = kvp.Value;
        }
