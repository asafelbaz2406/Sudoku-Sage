...
public class SudokuGenerator : MonoBehaviour
{
    ...
    private int[,] solutionBoard = new int[GridSize, GridSize];
    private readonly int[,] playableBoard = new int[GridSize, GridSize];
    private List<Point> availablePositions;
    private Dictionary<Point, int> deletedPositions = new();
    ...
    private List<Point> GetAvailablePositions() => new(availablePositions);
    #endregion

    public Dictionary<Point, int> CopyDeletedPositions(Dictionary<Point, int> copyDeletedPositions) => new(copyDeletedPositions);
    public void CopySolutionBoardToPlayableBoard() => CopyBoard(solutionBoard, playableBoard);
    ...
    public void GenerateSudoku()
    {
        ...
        if (!FillSudoku(solutionBoard))
        {
            logger.Log("Failed to fill the board", this);
            return;
        }
        ...
        OnSudokuGenerated?.Invoke(playableBoard);
        ...
        */
        ...
        logger.Log("Sudoku generated", this);
    }


    private void CreateAvailablePositionsList()
    {
        availablePositions = new List<Point>();

        for (int row = 0; row < GridSize; row++)
        {
            for (int col = 0; col < GridSize; col++)
            {
                availablePositions.Add(new Point(row, col));
            }
        }
    }
    ...
    public void RemoveNumbers(int numberOfCellsToRemove, int maxAttempts = 3)
    {
        ...
        CopyBoard(solutionBoard, initialBoard);  // Copy the solved board to the initial state

        for (int attempt = 0; attempt < maxAttempts; attempt++)
        {
            // Reset to the initial board state for each attempt
            CopyBoard(initialBoard, playableBoard);

            int cellsRemoved = 0;
            List<Point> availablePositions = GetAvailablePositions();
            Dictionary<Point, int> tempDeletedPositionsDict = new();
            ...
            foreach (Point point in availablePositions)
            {
                ...
                {
                    int backup = playableBoard[row, col];
                    playableBoard[row, col] = 0;

                    if (HasUniqueSolution(playableBoard))
                    {
                        cellsRemoved++;
                    ...
                    {
                        playableBoard[row, col] = backup; // Restore if it does not have a unique solution
                    }
                }
            }
            ...
            if (cellsRemoved > bestCellsRemoved)
            {
                ...
                CopyBoard(playableBoard, bestBoardState);
                deletedPositions = CopyDeletedPositions(tempDeletedPositionsDict);
            }

            if (cellsRemoved >= numberOfCellsToRemove)
                break;

            logger.Log($"attempt: {attempt}  removed: {cellsRemoved}", this);
        }

        // Apply the best result to the final board
        CopyBoard(bestBoardState, playableBoard);
        logger.Log($"Successfully removed {bestCellsRemoved} cells in the best attempt out of {maxAttempts}.", this);
    }
